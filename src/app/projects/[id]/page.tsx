...
const ALL_PHASES = [
    { key: 'S01_客戶查詢', label: 'S01 客戶查詢', dept: '拓展部' },
...
export default function ProjectDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [meetings, setMeetings] = useState<MeetingEntry[]>([{ dateTime: '', location: '' }]);
                ...
                <div className="px-6 sm:px-8">
                    ...
                    <Tabs defaultValue="overview" className="w-full">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-start">
                                                                    ...
                                                                    <div className="space-y-0.5">
                                                                        {visibleLogs.map((log: any, li: number) => (
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <span className="text-[10px] font-bold text-[#0071E3] whitespace-nowrap">{log.userName}</span>
                                ...
                                <div className="space-y-5">
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('overview') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <HardHat className="h-4 w-4 text-emerald-500" /> 項目概覽
                                                </CardTitle>
                                            </CardHeader>
                                    ...
                                    {userRole === 'admin' && (STAGE_WIDGETS[project.stage] || []).includes('status') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <FileText className="h-4 w-4 text-violet-500" /> 項目狀態
                                                </CardTitle>
                                            </CardHeader>
                                            <CardContent className="space-y-4 px-5 pt-4 pb-5">
                                                <div className="space-y-1.5">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">目前狀態</label>
                                                    <Select
                                                    ...
                                                    >
                                                        <SelectTrigger className="h-9 text-xs bg-slate-50 border-slate-200/60">
                                                            <SelectValue />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                ...
                                                <div className="space-y-1.5">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">簽約日期</label>
                                                    <Input
                                                        type="date"
                                                        ...
                                                        className="h-9 text-xs bg-slate-50 border-slate-200/60 shadow-none focus-visible:ring-violet-500/20"
                                                    />
                                                </div>
                                            </CardContent>
                                        </Card>
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('construction_team') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <Users className="h-4 w-4 text-blue-500" /> 施工組合
                                                </CardTitle>
                                            </CardHeader>
                                            <CardContent className="px-5 pt-4 pb-5">
                                                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                                                    {[
                                                        ...
                                                        <div key={item.key} className="space-y-1">
                                                            ...
                                                            <div className="relative">
                                                                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-sm">{item.icon}</span>
                                                                <Input
                                                                    ...
                                                                    className="h-9 text-xs pl-8 bg-slate-50 border-slate-200/60 shadow-none focus-visible:ring-blue-500/20"
                                                                />
                                                            </div>
                                                        </div>
                                                    ))}
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('construction_progress') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <HardHat className="h-4 w-4 text-orange-500" /> 工程進度
                                                </CardTitle>
                                                <CardDescription className="text-[11px] text-slate-400 mt-0.5">點擊展開查看各階段工序</CardDescription>
                                            </CardHeader>
                                            <CardContent className="px-3 pt-3 pb-3">
                                                <div className="space-y-1">
                                                    {CONSTRUCTION_PHASES.map((phase) => {
                                                        ...
                                                        return (
                                                            <div key={phase.key} className="rounded-lg border border-slate-100 overflow-hidden">
                                                                {/* Phase header — accordion trigger */}
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setExpandedPhases(prev =>
                                                                        prev.includes(phase.key)
                                                                            ? prev.filter(k => k !== phase.key)
                                                                            : [...prev, phase.key]
                                                                    )}
                                                                    className={`w-full flex items-center justify-between px-3 py-2.5 text-left transition-colors ${isExpanded ? 'bg-slate-50' : 'hover:bg-slate-50/60'
                                                                        }`}
                                                                >
                                                                    <div className="flex items-center gap-2 min-w-0">
                                                                        <span className="text-sm shrink-0">{phase.icon}</span>
                                                                        <span className={`text-xs font-semibold truncate ${allDone ? 'text-emerald-700' : 'text-slate-700'
                                                                            }`}>{phase.label}</span>
                                                                    </div>
                                                                    <div className="flex items-center gap-2 shrink-0">
                                                                        {doneCount > 0 && (
                                                                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${allDone
                                                                    ...
                                                                    <div className="border-t border-slate-100 bg-white px-3 py-2 space-y-1">
                                                                        {phase.fields.map((field) => {
                                                                            ...
                                                                            return (
                                                                                <label
                                                                                    key={field.key}
                                                                                    className="flex items-center gap-2.5 py-1.5 px-1 rounded-md hover:bg-slate-50 cursor-pointer transition-colors group"
                                                                                >
                                                                                    <button
                                                                                        type="button"
                                                                                        onClick={() => togglePhaseTask(phase.key, field.key, checked)}
                                                                                        className="shrink-0"
                                                                                    >
                                                                                        {checked ? (
                                                                                    ...
                                                                                    <span className={`text-xs font-medium ${checked ? 'text-slate-400 line-through' : 'text-slate-600'
                                                                                        }`}>{field.label}</span>
                                                                                </label>
                                                                            );
                                                                        })}
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('meetings') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 flex flex-row items-center justify-between space-y-0 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <Clock className="h-4 w-4 text-amber-500" /> 約見記錄
                                                </CardTitle>
                                                <div className="flex items-center gap-1">
                                                    <Button
                                                        variant="ghost" size="sm"
                                                        onClick={() => setMeetings(prev => [...prev, { dateTime: '', location: '' }])}
                                                        className="h-7 gap-1 text-[10px] font-bold text-amber-600 hover:text-amber-700 hover:bg-amber-50 px-2"
                                                    >
                                                        <Plus className="h-3 w-3" /> 新增約見
                                                    </Button>
                                                    <Button variant="ghost" size="icon" onClick={saveDetails} disabled={savingDetails} className="h-7 w-7 text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded-md">
                                            ...
                                            <CardContent className="px-5 pt-4 pb-5">
                                                <div className="space-y-3">
                                                    {meetings.map((m, idx) => (
                                                        <div key={idx} className="border border-slate-200/80 rounded-lg p-3.5 bg-slate-50/30 space-y-2.5">
                                                            <div className="flex items-center justify-between">
                                                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">第 {idx + 1} 次約見{idx === 0 ? ' (初次)' : ''}</span>
                                                                {meetings.length > 1 && (
                                                                    <button
                                                            ...
                                                            <div className="space-y-1.5">
                                                                <label className="text-[10px] font-semibold text-slate-400">約定時間</label>
                                                                <Input
                                                                    type="datetime-local"
                                                                    value={m.dateTime}
                                                                    onChange={e => setMeetings(prev => prev.map((item, i) => i === idx ? { ...item, dateTime: e.target.value } : item))}
                                                                    className="h-8 text-xs bg-white border-slate-200/60 shadow-none focus-visible:ring-amber-500/20"
                                                                />
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <label className="text-[10px] font-semibold text-slate-400">見面地點</label>
                                                                <Input
                                                                    type="text"
                                                                    value={m.location}
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('design_links') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 flex flex-row items-center justify-between space-y-0 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold flex items-center gap-2 text-slate-800">
                                                    <LinkIcon className="h-4 w-4 text-blue-500" /> 設計連結
                                                </CardTitle>
                                            ...
                                            <CardContent className="space-y-3 px-5 pt-4 pb-5">
                                                <div className="space-y-1.5">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">平面圖連結</label>
                                                    <Input
                                                        value={floorPlanLink}
                                                        onChange={e => setFloorPlanLink(e.target.value)}
                                                        placeholder="https://drive.google.com/..."
                                                        className="h-9 text-xs bg-slate-50 border-slate-200/60 shadow-none focus-visible:ring-blue-500/20"
                                                    />
                                                ...
                                                <div className="space-y-1.5">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">SketchUp 3D</label>
                                                    <Input
                                                        value={sketchUpLink}
                                    ...
                                    {(STAGE_WIDGETS[project.stage] || []).includes('notes') && (
                                        <Card className="shadow-sm border-slate-200/80 rounded-xl overflow-hidden">
                                            <CardHeader className="pb-3 border-b border-slate-100/60 flex flex-row items-center justify-between space-y-0 px-5 pt-5">
                                                <CardTitle className="text-[13px] font-bold text-slate-800">附加備註</CardTitle>
                                            ...
                                            <CardContent className="px-5 pt-4 pb-5">
                                                <textarea
                                                    ...
                                                    placeholder="沒有備註內容"
                                                    className="w-full min-h-[100px] text-[13px] text-slate-600 leading-relaxed bg-slate-50/50 rounded-lg border border-slate-200/60 p-3 focus:outline-none focus:ring-2 focus:ring-slate-200 resize-none placeholder:italic placeholder:text-slate-400"
                                                />
                                            </CardContent>
                                        </Card>
                                    )}
                        ...
                        < TabsContent value="documents" className="mt-0 outline-none" >
                            <Card className="shadow-sm border-slate-200/60 h-full min-h-[500px]">
